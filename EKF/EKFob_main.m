...
eqns = [dS; dI; dD; dT1; dT2; dH; dE; dAlpha];
vars = [S I D T1 T2 H E alpha];
static_coefs = [beta gamma delta1 delta2 epsilon sigma1 sigma2 tau1 tau2 lambda]; % Coefs

%% Algorithm initialization

nstates = 8; % number of states - already augmented
nmeasures = 7; % number of measurements - already augmented
nparams = 1; % number of parameters that are estimated by the KF 

% States and Covariance initialization
x0A = [S_data(1); I_data(1); D_data(1); T1_data(1); T2_data(1); H_data(1); E_data(1); 0.6];
P0 = diag([5 50 5 5 5 5 5 50]);
...
w_dist = [w_alpha]; % vector of the process disturbances

Zmeas = [S_data; I_data; D_data; T1_data; T2_data; H_data; E_data];

J = jacobian(eqns, vars);

L = [zeros((nstates - size(w_dist,1)),nparams); eye(nparams) ];
...
guessed_vals = [beta_val, gamma_val, delta1_val, delta2_val, epsi_val,...
