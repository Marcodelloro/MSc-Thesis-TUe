...
% vectors for the computation - This Select what you want to compute

eqns = [dS; dI; dD; dT1; dT2; dH; dE; dDelta1; dDelta1; dSigma1; dSigma2; dLambda];
vars = [S I D T1 T2 H E delta1 delta2 sigma1 sigma2 lambda];
static_coefs = [alpha beta gamma epsilon tau1 tau2]; % Coefs

%% Algorithm initialization

nstates = 12; % number of states - already augmented
nmeasures = 7; % number of measurements - already augmented
nparams = nstates - nmeasures; % number of parameters that are estimated by the KF 

% States and Covariance initialization
x0A = [S_data(1); I_data(1); D_data(1); T1_data(1); T2_data(1); H_data(1); E_data(1); 0.002; 5e-4; 0.03; 0.01; 0.03];
P0 = diag([5 50 5 5 5 5 5 50 50 50 50 50]);
...
w_dist = [w_delta1 w_delta2 w_sigma1 w_sigma2 w_lambda]; % vector of the process disturbances
...
guessed_vals = [alpha_val, beta_val, gamma_val, epsi_val, tau1_val, tau2_val];
