...
eqns = [dS; dI; dD; dT1; dT2; dH; dE; dAlpha; dGamma; dDelta1; dDelta1];
vars = [S I D T1 T2 H E alpha gamma delta1 delta2];
static_coefs = [beta epsilon sigma1 sigma2 tau1 tau2 lambda]; % Coefs

%% Algorithm initialization

nstates = 11; % number of states - already augmented
nmeasures = 7; % number of measurements - already augmented
nparams = 4; % number of parameters that are estimated by the KF 

% States and Covariance initialization
x0A = [S_data(1); I_data(1); D_data(1); T1_data(1); T2_data(1); H_data(1); E_data(1); 0.6; 0.1; 0.002; 5e-4];
P0 = diag([5 50 5 5 5 5 5 50 50 50 50]);
...
w_dist = [w_alpha w_gamma w_delta1 w_delta2]; % vector of the process disturbances
...
guessed_vals = [beta_val, epsi_val,sigma1_val,...
    sigma2_val, tau1_val, tau2_val, lambda_val];
