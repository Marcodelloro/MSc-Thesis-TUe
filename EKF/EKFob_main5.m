...
vars = [S I D T1 T2 H E delta1 delta2 sigma1 sigma2 tau1 tau2];
static_coefs = [alpha beta gamma epsilon lambda]; % Coefs

%% Algorithm initialization

nstates = 13; % number of states - already augmented
nmeasures = 7; % number of measurements - already augmented
nparams = 6; % number of parameters that are estimated by the KF 

% States and Covariance initialization
x0A = [S_data(1); I_data(1); D_data(1); T1_data(1); T2_data(1); H_data(1); E_data(1); 0.002; 5e-4; 0.03; 0.01; 0.03; 0.008];
P0 = diag([5 50 5 5 5 5 5 50 50 50 50 50 50]);
...
w_dist = [w_delta1 w_delta2 w_sigma1 w_sigma2 w_tau1 w_tau2]; % vector of the process disturbances
...
guessed_vals = [alpha_val, beta_val, gamma_val, epsi_val,lambda_val];
